...
class MLPlay:
    def __init__(self, side):
        ...
        if self.side == "1P":
            with open(os.path.join(os.path.dirname(__file__),'save','1p.pickle'), 'rb') as f:
                self.model = pickle.load(f)
        else:
            with open(os.path.join(os.path.dirname(__file__),'save','2p.pickle'), 'rb') as f:
                self.model = pickle.load(f)
            ...
            if self.side == "1P":
                if scene_info["ball_speed"][1] > 0 :
                    ...
                    if scene_info["ball_speed"][0] > 0:
                        ...
                        direction = 3
                X = [scene_info["ball_speed"][0],scene_info["ball_speed"][1], scene_info["platform_1P"][0], scene_info["platform_2P"][0], scene_info["blocker"][0]]
                ...
                if scene_info["ball_speed"][1] > 0 :
                    ...
                    if scene_info["ball_speed"][0] > 0:
                        ...
                        direction = 3
                X = [scene_info["ball"][0], scene_info["ball"][1], direction,scene_info["ball_speed"][0],scene_info["ball_speed"][1], scene_info["platform_1P"][0], scene_info["platform_1P"][1], scene_info["platform_2P"][0], scene_info["platform_2P"][1]]
                X = np.array(X).reshape((1,-1))
                pred = self.model.predict(X)
